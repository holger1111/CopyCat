...
**Macht genau**: `copycat/__init__.py` erhält `__version__ = "3.0.0"` als zentrale Versionsquelle. `copycat/cli.py` implementiert `main()` als offiziellen CLI-Einstiegspunkt – er lädt Argumente, setzt den Log-Level und delegiert an die passende Funktion (`run_copycat`, `list_plugins`, `install_hook`, `merge_reports`, `diff_reports`, `watch_and_run`, `build_timeline`). `parse_arguments()` in `core.py` bekommt `--version` (gibt `CopyCat 3.0.0` aus). `json_export.py` liest die Version jetzt dynamisch aus `__version__` statt hartkodiert `"3.0"`. `pyproject.toml` (hatchling, PEP 621) definiert den PyPI-Namen `copycat-tool`, optionale Extras (`template`, `watch`, `html`, `pdf`, `ai`, `web`, `gui`, `all`) und den `[project.scripts]`-Eintrag `copycat = "copycat.cli:main"`. `CopyCat.py` wird zum reinen Legacy-Wrapper (`__main__` delegiert an `copycat.cli:main`). In `.github/workflows/ci.yml` übernimmt ein neuer `publish`-Job (tag-getriggert, Trusted Publishing) das automatische PyPI-Release. `Dockerfile` und die VS Code Extension werden ebenfalls auf die neue CLI umgestellt.

### MEILENSTEIN 41: DE/EN-Sprachauswahl für Reports und Oberflächen
**Warum sinnvoll**: CopyCat erzeugte seine Reports bislang praktisch nur auf Deutsch. Für internationale Teams, öffentliche Projekt-Dokumentation und konsistente Nutzung über CLI, GUI und Web war das zu eng. Eine zentrale Sprachumschaltung reduziert doppelte Pflege, macht Reports sofort exportierbar für deutsch- und englischsprachige Zielgruppen und verhindert, dass einzelne Ausgabekanäle auseinanderlaufen.
**Macht genau**: `copycat/i18n.py` führt eine zentrale Übersetzungsfunktion `get_tr(lang)` für Deutsch und Englisch ein. `core.py` erweitert die CLI um `--lang de|en` und akzeptiert zusätzlich `lang = de|en` aus `copycat.conf`. `get_plural()` in `copycat/utils/files.py` erzeugt sprachabhängige Singular-/Pluralformen. Alle Exporter (`txt`, `md`, `html`, `pdf`) sowie die Timeline-Ausgabe werden auf übersetzbare Strings umgestellt; der Timeline-/Diff-Kontext akzeptiert dabei sowohl deutsche als auch englische TXT-Muster. `CopyCat_Web.py` ergänzt ein Sprach-Dropdown im Formular, `CopyCat_GUI.py` eine Sprach-Kombobox mit Persistenz in der Konfigurationsdatei. Tests decken CLI, Config, Exporter und Timeline in beiden Sprachen ab; Gesamtstand: 1102 Tests, mypy strict weiter sauber.
